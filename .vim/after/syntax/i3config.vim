...
syn match BorderStyle /^\s*\(new_window\|new_float\|default_border\|default_floating_border\)\s\+\(none\|\(normal\|pixel\|enable\|floating\|border\)\(\s\+\d\+\)\?\)\s\?$/ contains=BorderStyleKeyword,number
